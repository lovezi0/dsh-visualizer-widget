...
// 设置命名空间，与 client 半 settings.plugin.item 的 key 一致。
...
export const Config = Schema.object({
  enabled: Schema.boolean().default(true).description("是否把可视化渲染指令注入系统提示词。"),
  prompt: Schema.string().default(DEFAULT_PROMPT).description("注入系统提示词的可视化渲染指令文本。"),
});
...
export function apply(ctx, config) {
  // 运行时配置源：初始为 cordis 组合配置，settings 服务就绪后切到用户覆盖层。
  let source = () => config;

  // 设置集成：在 DSH 设置页暴露 visualizer-widget 配置面板；未挂载 settings 服务时回退到组合配置。
  ...
  ctx.systemPrompt.section({
    ...
    text: () => {
      const c = source();
      return c && c.enabled !== false ? (typeof c.prompt === "string" ? c.prompt : DEFAULT_PROMPT) : "";
    },
  });
